...
async function setElements() {
  ...
  console.log(isAuthenticatedResult, hasExistingVault, handshakeComplete);
  if (!isAuthenticatedResult && hasExistingVault) {
    ...
    const loginDetailsCount = await sendLoginDetailsCountRequest();
    console.log(loginDetailsCount);

    // Let's retrieve the first batch of passwords.
    const loginDetails = await sendLoginDetailsViewRequest(1);
    console.log(loginDetails);

    populatePasswordTable(loginDetailsCount, loginDetails);
...
function populatePasswordTable(count, loginDetails) {
